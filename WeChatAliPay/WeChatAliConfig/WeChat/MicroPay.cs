...
namespace WxPayAPI
{
    public class MicroPay
    {
        ...
        */
        ...
        public static string Run(string body, string total_fee, string auth_code, string out_trade)
        {
            //Log.Info("MicroPay", "Micropay is processing...");
            Log.Info("微信支付", "微信支付处理中...");

            WxPayData data = new WxPayData();
            data.SetValue("auth_code", auth_code);//授权码
            data.SetValue("body", body);//商品描述
            data.SetValue("total_fee", int.Parse(total_fee));//总金额
            //out_trade_no = WxPayApi.GenerateOutTradeNo();   //订单号
		    ...
		    result.GetValue("err_code").ToString() != "SYSTEMERROR")
		    {
                ...
                return result.ToPrintStrWinForm();
		    }
		    ...
		    while(queryTimes-- > 0)
		    {
                ...
                else if(succResult == 1)
                {
                    ...
                    return queryResult.ToPrintStrWinForm();
			    }
